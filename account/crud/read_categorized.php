<?php
    ...
    if($conn) {
        $query = "call spRetrieveAccountsByCategory();";
        ...
        if($data) {
            if($data->num_rows > 0) {
                ...
                while($row = $data->fetch_assoc()) {
                    ...
                    $result .= "{'Id': $id, 'Shortcode': '$shortcode', 'Title': '$title', 'CategoryId': $categoryId, 'Category': '$category', 'CategoryColor': '$categoryColor', 'CategoryOrder': $categoryOrder, 'AccountNumber': $accountNumberValue, 'BankIcon': $bankIconValue, 'Status': '$status'},";
                }
                $result = rtrim($result, ",");
                $result .= "]}";
            } else {
